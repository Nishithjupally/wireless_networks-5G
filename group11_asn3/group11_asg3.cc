...
main (int argc, char *argv[]) 
{
  ...
  std::string macScheduler = "ns3::NrMacSchedulerTdmaRR";
  double speed = 0;
  ...
  cmd.AddValue ("scheduler",
                "MAC Scheduler",
                macScheduler);
  cmd.AddValue ("speedUE",
                "To set speed in task3,4",
                speed);
  cmd.AddValue ("simutime",
  ...
  // Scheduler
  nrHelper->SetSchedulerTypeId (TypeId::LookupByName (macScheduler));
...
//  nrHelper->SetChannelConditionModelAttribute ("UpdatePeriod", TimeValue (MilliSeconds (0)));
  ...
  if(speed > 0){
    std::cout<<"Wrong\n";
  Ptr<ConstantRandomVariable> velocityGenerator = CreateObject<ConstantRandomVariable> ();
  velocityGenerator->SetAttribute ("Constant", DoubleValue (speed));
  mobility.SetMobilityModel ("ns3::RandomWalk2dMobilityModel","Speed",PointerValue (velocityGenerator),"Bounds", RectangleValue (Rectangle (-15000, 15000, -15000, 15000)));
...
if(ueNumPergNb==6)
{
  utPositionAlloc->Add (Vector (40, 1, ueHeight));
  utPositionAlloc->Add (Vector (50, -1, ueHeight));
  utPositionAlloc->Add (Vector (100,-1, ueHeight));
  utPositionAlloc->Add (Vector ((10+(rand()%2991)), 0, ueHeight));
  utPositionAlloc->Add (Vector ((10+(rand()%2991)), 0, ueHeight));
  utPositionAlloc->Add (Vector (50, 0, ueHeight));
  utPositionAlloc->Add (Vector (100, 0, ueHeight));
}

else //for question 8,9
{
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
      ...
      outFile << "  Packet Loss rate:   " << ((i->second.txPackets - i->second.rxPackets)*100.0)/(i->second.txPackets) << "\n";
      averagePacketLoss += ((i->second.txPackets - i->second.rxPackets)*100.0)/(i->second.txPackets);
  ...
  outFile << "  Mean packet loss: " << averagePacketLoss / stats.size () << "\n";
  //outFile << "  Stats size: " << stats.size () << "\n";
  outFile.close ();
  for (uint32_t i = 0; i < ueNodes.GetN (); ++i)
    {

  Ptr<UdpClient> clientApp = clientApps.Get (i)->GetObject<UdpClient> ();
