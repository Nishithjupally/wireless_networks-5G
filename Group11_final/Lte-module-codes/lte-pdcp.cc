...
std::vector < std::queue <
  ns3::LteRlcSapProvider::TransmitPdcpPduParameters >> v (1024);
bool
  flag = false;
...
LtePdcp::LtePdcp ():m_pdcpSapUser (0),
    m_rlcSapProvider (0),
    m_rnti (0), m_lcid (0), m_txSequenceNumber (0), m_rxSequenceNumber (0)
  {
    NS_LOG_FUNCTION (this);
    m_pdcpSapProvider =
      new LtePdcpSpecificLtePdcpSapProvider < LtePdcp > (this);
  ...
  TypeId LtePdcp::GetTypeId (void)
  {
    static TypeId
      tid =
      TypeId ("ns3::LtePdcp").SetParent < Object >
      ().SetGroupName ("Lte").AddTraceSource ("TxPDU",
  ...
  LtePdcp::Status LtePdcp::GetStatus ()
  {
    Status
      s;
...
////////////////////////////////////////
  ...
  for (int i = 0; i < 1024; ++i)
    {
      if (!v[i].empty ())
        {
    temp = true;
    m_rlcSapProvider->TransmitPdcpPdu (v[i].front ());
    v[i].pop ();
        }
    }
  if (!temp)
  ...
  void
  LtePdcp::DoTransmitPdcpSdu (LtePdcpSapProvider::
            TransmitPdcpSduParameters params)
  {
    NS_LOG_FUNCTION (this << m_rnti << static_cast < uint16_t >
         (m_lcid) << params.pdcpSdu->GetSize ());
    Ptr < Packet > p = params.pdcpSdu;

    // Sender timestamp
    PdcpTag
    pdcpTag (Simulator::Now ());
  ...
  m_txSequenceNumber = 0;
    ...
    int
      index = rand () % 1024;
    v[index].push (txParams);
    if (flag == false)
      {
  flag = true;
  std::thread sendtorlc (&LtePdcp::pdcptorlc, this);
  //call function
  sendtorlc.join ();
      }
  }
