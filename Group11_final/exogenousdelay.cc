...
main (int argc, char *argv[])
{
  ...
  uint32_t udpPacketSizeULL = 128;
  uint32_t udpPacketSizeNormal = 1024;
  uint32_t lambdaULL = 10000;
  uint32_t lambdaeMbb = 1000;
  ...
  double centralFrequencyBand = 6e9;
  double bandwidthBand = 25e6;
  ...
  cmd.AddValue ("lambdaUll",
    ...
    lambdaULL);
  cmd.AddValue ("lambdaeMbb",
    "Number of UDP packets in one second for best effor traffic",
    lambdaeMbb);
  cmd.AddValue ("simTime", "Simulation time", simTime);
  cmd.AddValue ("numerology", "The numerology to be", numerology);
  cmd.AddValue ("centralFrequencyBand",
    "The system frequency to be used", centralFrequencyBand);
  cmd.AddValue ("bandwidthBand",
    "The system bandwidth to be used ", bandwidthBand);
  ...
  Config::SetDefault ("ns3::LteRlcAm::MaxTxBufferSize", UintegerValue (9999));
  ...
  OperationBandInfo band1 =
    ccBwpCreator.CreateOperationBandContiguousCc (bandConf1);
  ...
  idealBeamformingHelper->SetAttribute ("BeamformingMethod",
          TypeIdValue (DirectPathBeamforming::
                 GetTypeId ()));
  ...
  NetDeviceContainer ueLowLatNetDev =
    nrHelper->InstallUeDevice (ueLowLatContainer, allBwps);
  ...
  nrHelper->GetGnbPhy (enbNetDev.Get (0), 0)->SetAttribute ("TxPower",
                  DoubleValue (10 *
                   log10
                   ((bandwidthBand / totalBandwidth) * x)));
  ...
  ipv4RoutingHelper.GetStaticRouting (gridScenario.GetUserTerminals ().
              Get (j)->GetObject < Ipv4 > ());
   ...
   */
  uint16_t dlPorteMBB = 2345;
  uint16_t dlPortLowLat = 2346;

  ApplicationContainer serverApps;

  // The sink will always listen to the specified ports
  UdpServerHelper dlPacketSinkLowLat (dlPortLowLat);
  UdpServerHelper dlPacketSinkVoice (dlPorteMBB);

  // The server, that is the application which is listening, is installed in the UE
  serverApps.Add (dlPacketSinkLowLat.Install (ueLowLatContainer));
  serverApps.Add (dlPacketSinkVoice.Install (ueLowLatContainer));


  OnOffHelper dlClientLowLat ("ns3::UdpSocketFactory",
            InetSocketAddress (ueLowLatIpIface.
             GetAddress (0),
             dlPortLowLat));
  dlClientLowLat.SetAttribute ("PacketSize",
             UintegerValue (udpPacketSizeULL));
  dlClientLowLat.SetAttribute ("OnTime",
             StringValue
             ("ns3::ConstantRandomVariable[Constant=0.06]"));
  dlClientLowLat.SetAttribute ("OffTime",
             StringValue
             ("ns3::ExponentialRandomVariable[Mean=0.25]"));
  dlClientLowLat.SetAttribute ("DataRate", StringValue ("20Mbps"));

  // The bearer that will carry low latency traffic
  EpsBearer lowLatBearer (EpsBearer::DGBR_DISCRETE_AUT_SMALL);
  ...
  dlClientVoice.SetAttribute ("RemotePort", UintegerValue (dlPorteMBB));
  dlClientVoice.SetAttribute ("MaxPackets", UintegerValue (0xFFFFFFFF));
  dlClientVoice.SetAttribute ("PacketSize",
            UintegerValue (udpPacketSizeNormal));
  dlClientVoice.SetAttribute ("Interval",
            TimeValue (Seconds (1.0 / lambdaeMbb)));
  ...
  dlpfVoice.localPortStart = dlPorteMBB;
  dlpfVoice.localPortEnd = dlPorteMBB;
  ...
  for (uint32_t i = 0; i < ueLowLatContainer.GetN (); ++i)
    {
      ...
      nrHelper->ActivateDedicatedEpsBearer (ueDevice, lowLatBearer,
              lowLatTft);
    }
  ...
  for (uint32_t i = 0; i < ueLowLatContainer.GetN (); ++i)
    {
      ...
      // Activate a dedicated bearer for the traffic type
      nrHelper->ActivateDedicatedEpsBearer (ueDevice, eMBBBearer, voiceTft);
    }
      ...
      if (t.protocol == 17)
  {
    protoStream.str ("UDP");
      ...
      outFile << "Flow " << i->first << " (" << t.sourceAddress << ":" << t.
  sourcePort << " -> " << t.destinationAddress << ":" << t.
  destinationPort << ") proto " << protoStream.str () << "\n";
      outFile << "  Tx Packets: " << i->second.txPackets << "\n";
      outFile << "  Tx Bytes:   " << i->second.txBytes << "\n";
      outFile << "  TxOffered:  " << i->second.txBytes * 8.0 / flowDuration /
  1000.0 / 1000.0 << " Mbps\n";
      ...
      if (i->second.rxPackets > 0)
  {
    ...
    averageFlowThroughput +=
      i->second.rxBytes * 8.0 / flowDuration / 1000 / 1000;
    averageFlowDelay +=
      1000 * i->second.delaySum.GetSeconds () / i->second.rxPackets;

    outFile << "  Throughput: " << i->second.rxBytes * 8.0 /
      flowDuration / 1000 / 1000 << " Mbps\n";
    outFile << "  Mean delay:  " << 1000 *
      i->second.delaySum.GetSeconds () / i->second.rxPackets << " ms\n";
  ...
  outFile << "\n\n  Mean flow throughput: " << averageFlowThroughput /
    stats.size () << "\n";
  outFile << "  Mean flow delay: " << averageFlowDelay /
    stats.size () << "\n";
